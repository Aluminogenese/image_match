...
void basematcher::draw_result_view(cv::Mat& outputImage, const cv::Mat& leftImage, const cv::Mat& rightImage, std::vector<MatchPointPair>& matchPoints) {
    ...
    for (auto& match : matchPoints)
    {
        ...
        cv::circle(outputImage, pt1, 4, color, 1);
        cv::circle(outputImage, pt2, 4, color, 1);
        cv::line(outputImage, pt1, pt2, color, 1);
    }

//    outputImage.create(leftImage.rows, leftImage.cols + rightImage.cols, leftImage.type());
////#pragma omp parallel for
//    for (int i = 0; i < leftImage.rows; i++) {
//        const uchar* leftPtr = leftImage.ptr<uchar>(i);
//        uchar* outputPtr = outputImage.ptr<uchar>(i);
...
//        const uchar* rightPtr = rightImage.ptr<uchar>(i);
//        uchar* outputPtr = outputImage.ptr<uchar>(i);
